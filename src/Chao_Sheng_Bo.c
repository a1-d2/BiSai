...
//		sprintf((char *)uart_buf,"echo_start=%d",echo_start);
//		uint16_t send_len = (uint16_t)strlen((char *)uart_buf);
//		R_SCI_UART_Write(&g_uart0_ctrl,uart_buf,send_len);
    }
    else if(p_args->event == TIMER_EVENT_CAPTURE_B) // 捕获B：Echo 下降沿（高电平结束）
    {
        echo_end = p_args->capture;    // 记录结束时间
        capture_flag = 1;              // 标记捕获完成
//		sprintf((char *)uart_buf,"echo_end=%d,capture_flag=%d",echo_end,capture_flag);
//		uint16_t send_len = (uint16_t)strlen((char *)uart_buf);
//		R_SCI_UART_Write(&g_uart0_ctrl,uart_buf,send_len);
    }
}
...
void Chao_Sheng_Bo_GPT0_Init(void) // 超声波定时器初始化
{
	...
	R_GPT_Enable(&g_timer0_ctrl); // 打开输入捕获功能
...
//	while(state == 1)
//	{
//		R_IOPORT_PinRead(&g_ioport_ctrl, TRIG_PIN, &state);
